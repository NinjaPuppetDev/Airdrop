...
TOKEN_ADDRESS=$(forge create src/PizzaToken.sol:PizzaToken --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --legacy --zksync | awk '/Deployed to:/ {print $3}' )
...
# Get message hash
MESSAGE_HASH=$(cast call ${AIRDROP_ADDRESS} "getMessage(address,uint256)" ${DEFAULT_ANVIL_ADDRESS} 25000000000000000000 --rpc-url http://127.0.0.1:8011)
